...
func TestSaveNodesFullReplaceSemantics(t *testing.T) {
	...
	if err := SaveNodes(testDB, "nodes", rows); err != nil {
		t.Fatalf("SaveNodes: %v", err)
	}

	// 全量替换为 2 行：stale 行（uri3）必须被清除
	if err := SaveNodes(testDB, "nodes", rows[:2]); err != nil {
		t.Fatalf("SaveNodes replace: %v", err)
	}
	var count int
	if err := testDB.QueryRow("SELECT COUNT(*) FROM nodes").Scan(&count); err != nil {
	...
	var typ, name string
	var disabled bool
	if err := testDB.QueryRow("SELECT type, name, disabled FROM nodes WHERE raw_uri = 'uri2'").Scan(&typ, &name, &disabled); err != nil {
...
func TestSaveHealthAppendOnly(t *testing.T) {
	...
	if err := SaveHealth(testDB, "node_health", hs); err != nil {
		t.Fatalf("SaveHealth: %v", err)
	}
	// 追加式替换：只写 1 行时，另一行必须保留（不得 DELETE）
	if err := SaveHealth(testDB, "node_health", hs[:1]); err != nil {
		t.Fatalf("SaveHealth append: %v", err)
	}
	var count int
	if err := testDB.QueryRow("SELECT COUNT(*) FROM node_health").Scan(&count); err != nil {
		t.Fatalf("count: %v", err)
	}
	if count != 2 {
	...
	if err := SaveHealth(testDB, "node_health", hs2); err != nil {
		t.Fatalf("SaveHealth replace: %v", err)
	}
	var sc int
	if err := testDB.QueryRow("SELECT success_count FROM node_health WHERE raw_uri = 'uri1'").Scan(&sc); err != nil {
		t.Fatalf("scan: %v", err)
	}
	if sc != 9 {
...
func TestLoadRoundTrip(t *testing.T) {
	...
	if err := LoadNodesFull(testDB, "nodes", func(rawURI, typ, name string, disabled bool) {
	...
	gotHealth, err := LoadHealth(testDB, "node_health")
	if err != nil {
...
func TestUpsertDisabled(t *testing.T) {
	...
	if err := SaveNodes(testDB, "nodes", []NodeRow{{RawURI: "uri1", Type: "ss", Name: "n1"}}); err != nil {
		t.Fatalf("SaveNodes: %v", err)
	}
	if err := UpsertDisabled(testDB, "nodes", "uri1", true); err != nil {
		t.Fatalf("UpsertDisabled: %v", err)
	}
	var disabled bool
	if err := testDB.QueryRow("SELECT disabled FROM nodes WHERE raw_uri = 'uri1'").Scan(&disabled); err != nil {
		t.Fatalf("scan: %v", err)
	}
	if !disabled {
	...
	// 不存在的 URI 零影响，不报错
	if err := UpsertDisabled(testDB, "nodes", "ghost", false); err != nil {
...
func TestUpsertHealth(t *testing.T) {
	...
	if err := UpsertHealth(testDB, "node_health", h); err != nil {
...
func TestUpdateDisabledBatch(t *testing.T) {
	...
	if err := SaveNodes(testDB, "nodes", []NodeRow{
		{RawURI: "uri1", Name: "n1"},
		{RawURI: "uri2", Name: "n2"},
		{RawURI: "uri3", Name: "n3"},
	}); err != nil {
	...
	// 重复 URI 幂等
	if err := UpdateDisabledBatch(testDB, "nodes", []string{"uri1", "uri1", "uri3"}, true); err != nil {
		t.Fatalf("UpdateDisabledBatch: %v", err)
	}
	var count int
	if err := testDB.QueryRow("SELECT COUNT(*) FROM nodes WHERE disabled = 1").Scan(&count); err != nil {
		t.Fatalf("count: %v", err)
	}
	if count != 2 {
		t.Errorf("批量禁用数量=%d, want 2", count)
	}
	if err := UpdateDisabledBatch(testDB, "nodes", []string{"uri1"}, false); err != nil {
		t.Fatalf("UpdateDisabledBatch enable: %v", err)
	}
	var d int
	if err := testDB.QueryRow("SELECT disabled FROM nodes WHERE raw_uri = 'uri1'").Scan(&d); err != nil {
		t.Fatalf("scan: %v", err)
	}
	if d != 0 {
...
func TestDeleteDisabledNodesSemantics(t *testing.T) {
	...
	if err := SaveHealth(testDB, "node_health", []HealthRow{
	...
	removed, uris, err := DeleteDisabledNodes(testDB, "nodes", "node_health")
	if err != nil {
	...
	// 节点表：仅剩启用节点
	var nodeCount int
	if err := testDB.QueryRow("SELECT COUNT(*) FROM nodes").Scan(&nodeCount); err != nil {
	...
	var healthCount int
	if err := testDB.QueryRow("SELECT COUNT(*) FROM node_health").Scan(&healthCount); err != nil {
...
func TestDeleteNodesBatchSemantics(t *testing.T) {
	...
	if err := DeleteNodesBatch(testDB, "nodes", "node_health", del); err != nil {
	...
	// 保留行未被误删
	var sc int
	if err := testDB.QueryRow("SELECT success_count FROM node_health WHERE raw_uri = 'uri700'").Scan(&sc); err != nil {
...
func TestDeleteNodesBatchNilAndEmpty(t *testing.T) {
	...
	if err := DeleteNodesBatch(testDB, "nodes", "node_health", nil); err != nil {
