...
func TestCreateDialer_ReturnsUniqueBoxes(t *testing.T) {
	...
	d := &singDialer{
		cfg:        cfg,
		entry:      newEntryBoxPoolManager(DialerDeps{}),
...
func TestEntryProxyAddr_EmptyByDefault(t *testing.T) {
	m := newEntryBoxPoolManager(DialerDeps{})
...
func TestStartEntryBox_UsesEphemeralPort(t *testing.T) {
	m := newEntryBoxPoolManager(DialerDeps{Cache: NewIRCache()})
	box, addr, err := m.startEntryBox("socks5://127.0.0.1:1080")
	if err != nil {
