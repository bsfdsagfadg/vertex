...
// RunRace runs a hedge race across multiple candidate exit nodes.
//
// It handles:
...
func RunRace[T any](ctx context.Context, pool NodePool, cfg config.ConfigProvider,
	...
	var zero T
	cands := pool.SelectForParallel(cfg.ParallelPoolSize(), cfg.DebugMode())
	...
	if !cfg.ParallelPoolDelayDynamic() {
		...
		delay = time.Duration(pool.GetAverageLatency()) * time.Millisecond
