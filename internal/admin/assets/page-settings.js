const SETTINGS_FIELDS = [
  ...
  { k: 'race_timeout', label: '单节点竞速超时（秒）', type: 'number', max: 1800, min: 0, group: 'pool', desc: '每个候选节点等待首个可判定结果的最大时间；流式请求即等待首个合法上游包（0 = 不单独限制，默认 60 秒）' },
  { k: 'stream_idle_timeout_seconds', label: '包间空闲超时（秒）', type: 'number', max: 1800, min: 1, group: 'pool', desc: '包间空闲可等待的最大时间；收到任何合法 SSE 包后重新计时，默认 20 秒。' },
  { k: 'recaptcha_try_entry_or_direct', label: '优先前置/直连抓取 recaptchaToken', type: 'bool', group: 'pool', desc: '优先通过入口代理或服务器直连获取 recaptchaToken，失败后回退请求节点。' },
  ...
  { k: 'max_request_mb', label: '最大请求体 (MB)', type: 'number', group: 'core', desc: '请求体超过此大小将返回 413，默认 128' },
  { k: 'max_spill_mb', label: '最大内存缓冲 (MB)', type: 'number', group: 'core', desc: '上传大文件时，超过此大小将写入磁盘，默认 32' },
